...
test_variable_data = variable_data_m %>%
  filter(date_debut_arret == "2023-01-01" & departement == "75")
...
# This only needs to be re-run if you want to change the number of controls per case
# or the delay between control and case dates
...
case_crossover_data <- case_data_m %>%
  ...
  select(status, pseudo_id_indiv, departement, date_debut_arret, day_debut_arret,
         ...
         pm25_roll, NO2_roll, O3_roll)

test_case_data = case_crossover_data %>%
  filter(date_debut_arret == "2023-01-01" & departement == "75")
...
control_data <- control_dates %>%
  ...
  mutate(status = 0) %>%
  mutate(annee = year(date_debut_arret)) %>%
  select(status, pseudo_id_indiv, annee, departement, date_debut_arret, TM_mean_roll, pm25_roll, NO2_roll, O3_roll, day_debut_arret) %>%
...
test_control_data = control_data %>%
  filter(date_debut_arret == "2023-01-01" & departement == "75")

# Combine case and control data
# on ne garde que les cas qui ont le bon nombre
# ATTENTION le 3 doit changer ici si jamais on a + ou - de contrôles par cas
...
summary(model)
# Exponentiate coefficients to get odds ratios and get p values
exp(coef(model))*100
